...
function setup() {
  ...
  for (let row = 0; row < num_rows; row++) {
    for (let col = 0; col < num_bricks_per_row; col++) {
      bricks.push({
        x: col * (brick_width + padding_left) + 20, // added this 5 for spacing between bricks
        y: row * (brick_height + padding_top) + 20, // added 5 for spacing between rows
        width: brick_width,
        height: brick_height,
        isBroken: false
      });
...
function draw() {
  ...
  circle(ball_x, ball_y, ball_diameter/2);
